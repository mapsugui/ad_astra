...
**Superceded by the multi-archive runner:** re-run through `python -m cygnus.multi` on 2026-09-25, steps fetch_products, calibrate_screen, known_signal_recovery, residual_screen, period_aliases, prior_art (ledger fetch_products #1501, calibrate_screen #1502, known_signal_recovery #1503, residual_screen #1504, period_aliases #1505, prior_art #1506). Same retrieved products, same science outputs: entries, depths, k-star calibration and aliases match the production-runner results recorded here; the multi runner additionally records a screen-suitability census and a red-noise systematics model per light curve. The record is regenerated by the multi runner; nothing in the review changes.
...
| Check | State | Note |
|---|---|---|
| Target-to-Gaia identification (proper motion propagated) | passed | TOI-285.01: Gaia DR3 4764216563561182336 at 0.00" (propagated 2016.0 → J2015.5; 0.03" unpropagated, proper-motion shift 0.03") |
| Stellar priors (Gaia colour and parallax) | passed | TOI-285.01: Teff 3912 K, R* 0.61 ± 0.05, M* 0.62 ± 0.06, ρ* 2.66 ± 0.69 ρ☉ (dwarf sequence, M_G 7.77, no extinction) |
| Blend and dilution census (Gaia DR3 cone) | inconclusive | TOI-285.01: 1 Gaia neighbour(s) within 52.5", contamination 1.08%; depth 2426 ppm (catalogue depth); 1 could produce it if fully eclipsed (brightest 4764216456185576448, 48.7", ΔG 4.91); a centroid test is needed |
...
| Variable-catalogue collision (VSX) | passed | TOI-285.01: no VSX entry within 10" |
| Object-class guard (SIMBAD) | passed | TOI-285.01: TOI-285 otype PM* (star_or_other) at 0.9" |
