...
**Superceded by the multi-archive runner:** re-run through `python -m cygnus.multi` on 2026-09-25, steps fetch_products, calibrate_screen, known_signal_recovery, residual_screen, period_aliases, prior_art (ledger fetch_products #1682, calibrate_screen #1683, known_signal_recovery #1684, residual_screen #1685, period_aliases #1686, prior_art #1687). Same retrieved products, same science outputs: entries, depths, k-star calibration and aliases match the production-runner results recorded here; the multi runner additionally records a screen-suitability census and a red-noise systematics model per light curve. The record is regenerated by the multi runner; nothing in the review changes.
...
| Check | State | Note |
|---|---|---|
| Target-to-Gaia identification (proper motion propagated) | passed | TOI-6675.01: Gaia DR3 1140358282886912256 at 0.00" (propagated 2016.0 → J2015.5; 0.01" unpropagated, proper-motion shift 0.01") |
| Stellar priors (Gaia colour and parallax) | passed | TOI-6675.01: Teff 6532 K, R* 1.37 ± 0.11, M* 1.26 ± 0.13, ρ* 0.49 ± 0.13 ρ☉ (dwarf sequence, M_G 3.53, no extinction) |
| Blend and dilution census (Gaia DR3 cone) | inconclusive | TOI-6675.01: 4 Gaia neighbour(s) within 52.5", contamination 4.11%; depth 1249 ppm (measured depth of the recovered catalogued transit); 2 could produce it if fully eclipsed (brightest 1140357904929792512, 50.2", ΔG 3.46); a centroid test is needed |
...
| Variable-catalogue collision (VSX) | passed | TOI-6675.01: no VSX entry within 10" |
| Object-class guard (SIMBAD) | passed | TOI-6675.01: TYC 4530-25-1 otype * (star_or_other) at 0.2" |
