...
**Superceded by the multi-archive runner:** re-run through `python -m cygnus.multi` on 2026-09-25, steps fetch_products, calibrate_screen, known_signal_recovery, residual_screen, period_aliases, prior_art (ledger fetch_products #1615, calibrate_screen #1616, known_signal_recovery #1617, residual_screen #1618, period_aliases #1619, prior_art #1620). Same retrieved products, same science outputs: entries, depths, k-star calibration and aliases match the production-runner results recorded here; the multi runner additionally records a screen-suitability census and a red-noise systematics model per light curve. The record is regenerated by the multi runner; nothing in the review changes.
...
| Check | State | Note |
|---|---|---|
| Target-to-Gaia identification (proper motion propagated) | passed | TOI-5893.01: Gaia DR3 1743373271792149760 at 0.00" (propagated 2016.0 → J2015.5; 0.01" unpropagated, proper-motion shift 0.01") |
| Stellar priors (Gaia colour and parallax) | passed | TOI-5893.01: Teff 5724 K, R* 1.10 ± 0.09, M* 1.06 ± 0.11, ρ* 0.80 ± 0.21 ρ☉ (dwarf sequence, M_G 4.33, no extinction) |
| Blend and dilution census (Gaia DR3 cone) | inconclusive | TOI-5893.01: 17 Gaia neighbour(s) within 52.5", contamination 11.76%; depth 7997 ppm (catalogue depth); 4 could produce it if fully eclipsed (brightest 1743373374871366784, 47.0", ΔG 2.96); a centroid test is needed |
...
| Variable-catalogue collision (VSX) | passed | TOI-5893.01: no VSX entry within 10" |
| Object-class guard (SIMBAD) | passed | TOI-5893.01: UCAC4 495-138480 otype * (star_or_other) at 0.2" |
