...
- **MPC:** the public site `minorplanetcenter.net` is unreachable from this host (DNS resolves, TCP times out on 80/443), but the **data host answers** [V 2026-09-25]: `GET https://data.minorplanetcenter.net/api/get-obs` with a JSON body `{"desigs": ["<name>"]}` and `Content-Type: application/json` returns observations (also `/api/obscodes`; `/api/` returns a hello). The experimental MPC adapter uses this route; treat the main-site check as not tested until it answers.
