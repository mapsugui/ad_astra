"""Lead vetting for known-object campaigns: ``python -m experimental.cygnus_multi vet campaigns/<slug>.yaml``.
...
records a ledger run (script ``cygnus_multi:<id>:lead_vetting``). It never changes the record's
...
def summary_md(r: dict) -> str:
    L = [f"# Lead vetting: {r['target']} (TIC {r['tic']})", "",
         "Generated by `python -m experimental.cygnus_multi vet`. States are the tool's; the reviewer's reading goes in REPORT.md. "
