...
def step_fetch_products(ctx, params: dict) -> dict:
    ...
    for p in wanted:
        ...
        path = next((d / fname for d in dirs if (d / fname).is_file()), None)
        fetched = False
        if path is None:
        ...
        if not ctx.ledger_has_product(archive, pid):
            ctx.ledger.add_product(archive, pid, url=p.get("url") or MAST_DOWNLOAD.format(pid=pid), local_path=path,
    ...
    lightcurves = [pid for pid, v in out.items() if v["kind"] == "lightcurve"]
...
def step_residual_screen(ctx, params: dict) -> dict:
    ...
    if single_pids:
        ...
        for pid in single_pids:
            ...
            if st and (strongest is None or st["trial_corrected_fap"] < strongest["trial_corrected_fap"]):
        ...
        if strongest is None:
            ...
            if fap is None:
                ...
                state = "passed" if fap <= 0.01 else "inconclusive" if fap <= 0.1 else "failed"
...
def step_source_checks(ctx, params: dict) -> dict:
    ...
    for archive, states in sorted(by_archive.items()):
        ...
        failing = [f"{pid}:{c['name']}" for pid, cs in out.items() for c in cs if c["state"] != "passed"]
...
def step_astrometric_vetting(ctx, params: dict) -> dict:
    ...
    for t in ctx.targets():
        ...
        if res.get("top_mass_function_msun") is not None:
            ctx.measure("nss_photocentre_mass_function", res["top_mass_function_msun"], unit="Msun",
