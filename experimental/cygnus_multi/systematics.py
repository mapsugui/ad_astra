...
def event_significance(lc, channel: np.ndarray, resid: np.ndarray, rednoise: RedNoise, *, t_mid: float, dur_d: float,
    ...
    # Red noise inflates the per-epoch variance by the correlation length; the
    # parametric z applies that inflation, the empirical p is floor-limited by the
    # number of random epochs. Both are reported; the state uses the parametric
    # trial-corrected FAP and the empirical floor is a sanity check.
    inflation = (max(1.0, (tau or 0.0) / cad_d)) ** 0.5 if cad_d > 0 else 1.0
    z_eff = z / inflation if z is not None else None
    ...
    return {"duration_label": duration_label, "box_statistic": stat, "robust_z": z,
            ...
            "interpretation": "empirical dip statistic at the event vs random epochs of the same light curve; "
                              "parametric p from the tau-inflated robust z, corrected for the independent "
                              "resolution elements at this duration. The empirical p cannot go below 1/(n_random+1)."}
...
def product_summary(lc, channel: np.ndarray, events: list[dict], *, seed: int = 0, n_random: int = 300) -> dict:
    ...
    ``events`` items need ``mid_time_BJD_like``, ``max_cadences`` and (optionally)
    ``duration_h``; the duration is derived from cadences when not given.
    """
    ...
    for e in events:
        if e.get("duration_h"):
            ...
            dur_d = max(2, int(e.get("max_cadences", 2))) * lc.cadence_s / 86400
