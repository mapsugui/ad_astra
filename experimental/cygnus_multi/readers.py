...
def _read_votable_lightcurve(path: Path) -> LightCurve:
    ...
    tcol = next((names[c] for c in ("mjd", "time", "hjd", "bjd", "jd") if c in names), None)
    if tcol is None:
    ...
    return LightCurve(path=path, time=time, fluxes=fluxes, quality=quality, flux_errs=errs,
                      bjdref=0.0, cadence_s=0.0, time_scale="UTC/MJD as supplied", time_unit="d",
                      primary={"OBJECT": "ZTF/IRSA VOTable"}, table_header={}, channels_order=tuple(order))
...
def _read_csv_lightcurve(path: Path) -> LightCurve:
    ...
    return LightCurve(path=path, time=time, fluxes=fluxes, quality=quality, flux_errs=errs,
                      bjdref=0.0, cadence_s=120.0, primary={}, table_header={}, channels_order=tuple(order))
