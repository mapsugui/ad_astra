"""Archive adapter framework for the experimental multi-archive pipeline.
...
:func:`available` / :func:`all_adapters`. ``cygnus_multi.archives`` imports each
...
class ProductRef:
    ...
    ``format`` selects the reader (see ``cygnus_multi.readers``); ``kind`` says
...
class ArchiveAdapter(ABC):
    ...
    name: str = ""
    #: human description shown in ``cygnus_multi.archives.summary()``
    description: str = ""
    ...
    def read(self, path: Path, ref: ProductRef) -> Any:
        """Read a downloaded product. Default: dispatch through ``cygnus_multi.readers``."""
    ...
    def source_checks(self, target: Target | None, ref: ProductRef, path: Path,
        ...
        The default dispatches by product kind through ``cygnus_multi.source_checks``
