...
``fetch``/``read`` from :class:`cygnus_multi.archives.base.ArchiveAdapter`.
...
class MastAdapter(ArchiveAdapter):
    ...
    def source_checks(self, target, ref, path, product=None):
        ...
        if (product or {}).get("kind") == "lightcurve":
            try:
                ...
                checks.append(SourceCheck("MAST quality flags", "passed" if bad == 0 else "inconclusive",
                                          f"{bad} of {lc.time.size} cadence(s) with QUALITY != 0 (excluded from the screen)"))
    ...
    def discover(self, target: Target, *, limit: int = 5, collection: str = "TESS",
        ...
        params = {"target_name": key, "obs_collection": collection, "dataproduct_type": "timeseries"}
        if provenance:
            params["provenance_name"] = provenance
        try:
            obs = self._mashup("Mast.Caom.Cone" if target.ra_deg is not None else "Mast.Caom.Filtered", params)
...
class NedAdapter(ArchiveAdapter):
    ...
    def discover(self, target: Target, *, limit: int = 200, radius_arcsec: float = 60.0, **opts) -> list[ProductRef]:
        r = radius_arcsec / 3600
        adql = (f"SELECT TOP {int(limit)} objname, ra, dec, objtype FROM objdir WHERE "
                f"1=CONTAINS(POINT('ICRS', ra, dec), CIRCLE('ICRS', {target.ra_deg:.7f}, {target.dec_deg:.7f}, {r:.7f}))")
        try:
