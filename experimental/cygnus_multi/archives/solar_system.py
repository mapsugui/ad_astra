...
class SbdbAdapter(_ContextAdapter):
    ...
    def discover(self, target: Target, *, limit: int = 1, **opts) -> list[ProductRef]:
        try:
            r = self.http_get(SBDB_API, params={"sstr": target.name, "orb": "1"}, timeout=60.0)
            r.raise_for_status()
            txt = r.text
        except Exception as exc:  # noqa: BLE001
...
class SkybotAdapter(_ContextAdapter):
    ...
    def discover(self, target: Target, *, limit: int = 100, epoch_iso: str | None = None,
        ...
        try:
            ...
            table = Skybot.cone_search(coord, radius=u.Quantity(radius_arcsec, u.arcsec), epoch=Time(epoch_iso, scale="utc"))
            rows = [] if table is None else [{c: table[c][i] for c in table.colnames} for i in range(len(table))]
