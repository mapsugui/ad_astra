...
- **`config.py`** — single source of truth for paths: worktree (`[private path]`),
  scratch root (`[private path]`, env `CYGNUS_SCRATCH`,
...
`[private Drive store]` via rclone (see `DATA_SOURCES.md`).
...
- Python 3.13 venv: `[private path]` (disposable; recreate
