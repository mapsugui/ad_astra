...
**Superceded by the multi-archive runner:** re-run through `python -m cygnus.multi` on 2026-09-25, steps fetch_products, calibrate_screen, known_signal_recovery, residual_screen, period_aliases, prior_art (ledger fetch_products #1447, calibrate_screen #1448, known_signal_recovery #1449, residual_screen #1450, period_aliases #1451, prior_art #1452). Same retrieved products, same science outputs: entries, depths, k-star calibration and aliases match the production-runner results recorded here; the multi runner additionally records a screen-suitability census and a red-noise systematics model per light curve. The record is regenerated by the multi runner; nothing in the review changes.
...
| Check | State | Note |
|---|---|---|
| Target-to-Gaia identification (proper motion propagated) | passed | TOI-2436.01: Gaia DR3 1689544206152233472 at 0.00" (propagated 2016.0 → J2015.5; 0.03" unpropagated, proper-motion shift 0.03") |
| Stellar priors (Gaia colour and parallax) | passed | TOI-2436.01: Teff 6287 K, R* 1.23 ± 0.10, M* 1.18 ± 0.12, ρ* 0.63 ± 0.16 ρ☉ (dwarf sequence, M_G 3.87, no extinction) |
| Blend and dilution census (Gaia DR3 cone) | inconclusive | TOI-2436.01: 3 Gaia neighbour(s) within 52.5", contamination 0.44%; depth 368 ppm (catalogue depth); 2 could produce it if fully eclipsed (brightest 1689543896915487616, 51.3", ΔG 6.00); a centroid test is needed |
...
| Variable-catalogue collision (VSX) | passed | TOI-2436.01: no VSX entry within 10" |
| Object-class guard (SIMBAD) | passed | TOI-2436.01: BD+72   581 otype PM* (star_or_other) at 1.1" |
