...
- Archive: MAST public product downloads; source URLs and product dataURIs are in `docs/tier1_pack/MASTER_MANIFEST.csv`; transfer used the existing scoped `[private Drive store]` Drive location, one file at a time with `rclone copyto` (not sync). Scratch is `[private path]` and is disposable; no originals altered.
...
```text
[private path] tools\analyze_tess_residual.py [private path] --out reports\tess-wasp12-residual-01\sector20
[private path] tools\analyze_tess_residual.py [private path] --out reports\tess-wasp12-residual-01\sector43
```
