...
Every check returns a :class:`~cygnus.multi.archives.base.SourceCheck` with one of
...
def lightcurve_checks(path: Path, *, fmt: str, source: str = "light curve") -> list[SourceCheck]:
    ...
    if lc.time_scale and "not established" in str(lc.time_scale):
        out.append(SourceCheck(f"{source} time standard", "not_tested", str(lc.time_scale)))
    elif lc.time_scale and "not barycentric" in str(lc.time_scale):
        out.append(SourceCheck(f"{source} time standard", "inconclusive", f"{lc.time_scale}; epochs differ from BJD_TDB"))
    elif lc.time_scale:
