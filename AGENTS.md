...
- **Google Drive is connected, but each harness reaches it its own way**: an rclone remote whose name is local configuration (`gdrive:` in the workstation's Claude Code harness, `cygnus:` in another), or a Colab `drive.mount()`. Never hard-code a remote name: name locations by their path inside the Drive folder `Cygnus/`, resolve the route with `python -m cygnus.storage where`, and record every stored location in `storage/locations.jsonl` (`docs/STORAGE.md`). Token access differs by harness and must be observed, not assumed: the workstation's `gdrive:` token is `drive.file`-scoped and does **not** see folders Colab's mount wrote (checked 2026-09-26), while another harness's token was observed to see them. **Drive paths outside `Cygnus/` are off-limits by policy, whatever a token can reach: nothing outside `Cygnus/` may be read, moved, deleted or overwritten, and every change is verified before it is asserted.** All persistent Cygnus data lives under `Cygnus/`. Bulky local downloads and intermediates go to `D:\AO_Artifacts\cygnus_scratch\` (nonsynced, safe to delete). rclone's shared Google client ID is being retired during 2026: if authorization errors suddenly appear, re-run the OAuth flow and tell the user rather than hiding the failure. Keep credentials out of logs and committed files, and verify every claimed upload before asserting success.
