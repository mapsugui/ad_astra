...
2. **Run analyses as campaigns.** Put parameters in a `campaigns/<id>.yaml` spec and run `python -m cygnus.campaign run` (`docs/CAMPAIGNS.md`): steps are ledgered, resumable and regenerate the sky record. Write the report and search log, then run `python -m pytest -q`; the suite fails if a campaign spec or report has no valid record.
